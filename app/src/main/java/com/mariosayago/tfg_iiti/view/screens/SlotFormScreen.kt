...
fun SlotFormScreen(
    ...
    // 3) Estados locales para el dropdown
    var expanded by remember { mutableStateOf(false) }
    var selectedPid by remember { mutableLongStateOf(sp?.slot?.productId ?: products.first().id) }
    var stockText by remember { mutableStateOf(sp?.slot?.currentStock?.toString() ?: "") }
    ...
    Column(Modifier.padding(16.dp)) {
        ...
        Button(onClick = {
            sp?.let {
                // actualiza el slot con el producto y stock elegidos
                slotVm.update(
                    it.slot.copy(
                        productId = selectedPid,
                        currentStock = stockText.toIntOrNull() ?: it.slot.currentStock
                    )
                )
            }
            onSave()
